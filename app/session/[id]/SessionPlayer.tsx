...
export default function SessionPlayer({ plan, userId, isPro, voiceCoachingEnabled, sessionsThisWeek, partialSession }: Props) {
  ...
  function confirmSkip() {
    ...
    if (curIdx < exercises.length - 1) {
      ...
      setRepCount(0); setHoldElapsed(0); autoAdvancedRef.current = false
      setShowNameOverlay(true)
      setPaused(false)
      primeActiveStage()
      if (!isPro) announceExerciseStart(nextIdx)
        ...
        <div className="flex gap-3 w-full max-w-xs">
          ...
          {!isLast && (
            <button onClick={() => {
              ...
              setCurrentEx(nextIdx); setRepCount(0); setHoldElapsed(0); autoAdvancedRef.current = false
