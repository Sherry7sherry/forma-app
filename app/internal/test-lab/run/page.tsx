...
export default async function DirectedRunPage({
  searchParams,
}: {
  ...
  try {
    const scenario = parseTestScenario(new URLSearchParams(
      Object.entries(query).filter(([, value]) => value !== undefined) as [string, string][],
    ))
    parsedScenario = scenario
    movement = getTestableMovement(scenario.movementId)
  } catch {
    movement = undefined
  }

  if (!movement || !parsedScenario) {
    return (
      <main className="p-6">
        <h1 className="text-xl font-semibold">Invalid test scenario</h1>
        <Link href="/internal/test-lab" className="btn-primary mt-4">
          Back to test lab
        </Link>
      </main>
    )
  ...
  return movement.kind === 'assessment'
    ? <DirectedAssessmentRunner movement={movement} scenario={parsedScenario} />
    : <DirectedExerciseRunner movement={movement} scenario={parsedScenario} />
}
